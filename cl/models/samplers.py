...
class Sampler(ABC):
    ...
    def add_intraview_negs(anchor, sample, pos_mask, neg_mask):
        num_instances = anchor.size(0)
        device = anchor.device
        intraview_pos_mask = torch.zeros_like(pos_mask, device=device)
        intraview_neg_mask = torch.ones_like(pos_mask, device=device) - torch.eye(num_instances, device=device)
...
class SameScaleSampler(Sampler):
    ...
    def sample(self, anchor, sample, *args, **kwargs):
        ...
        num_instances = anchor.size(0)
        device = anchor.device
        pos_mask = torch.eye(num_instances, dtype=torch.float32, device=device)
        neg_mask = 1. - pos_mask
...
class CrossScaleSampler(Sampler):
    ...
    def sample(self, anchor, sample, batch=None, neg_sample=None, use_gpu=True, *args, **kwargs):
        num_bags = anchor.shape[0]  # M
        num_instances = sample.shape[0]   # N
        device = sample.device

        if neg_sample is not None:
            assert num_bags == 1
            assert sample.shape == neg_sample.shape
            pos_mask1 = torch.ones((num_bags, num_instances), dtype=torch.float32, device=device)
            pos_mask0 = torch.zeros((num_bags, num_instances), dtype=torch.float32, device=device)
            pos_mask = torch.cat([pos_mask1, pos_mask0], dim=1)     # M * 2N
            sample = torch.cat([sample, neg_sample], dim=0)         # 2N * K
        else:
            assert batch is not None
            if use_gpu:
                ones = torch.eye(num_instances, dtype=torch.float32, device=device)     # N * N
                pos_mask = scatter(ones, batch, dim=0, reduce='sum')                # M * N
            else:
                pos_mask = torch.zeros((num_bags, num_instances), dtype=torch.float32).to(device)
